...
def yolo_format(
    ...
    dataset = {
        ...
        "train": (
            f"{yolo_dir}/images/{str(preprocessed_dirs_stems[0])}/"
            if len(preprocessed_dirs) == 1
            else [f"{yolo_dir}/images/{str(d)}" for d in preprocessed_dirs_stems]
        ),
    }
    if len(val_dirs_stems) > 0:
        dataset["val"] = (
            f"{yolo_dir}/images/{str(val_dirs_stems[0])}/"
            if len(val_dirs_stems) == 1
            else [f"{yolo_dir}/images/{str(d)}" for d in val_dirs_stems]
        )
