...
def predict(
    ...
    Args:
        ...
        remove_images: Bool indicating whether to delete prediction images
            after they were georeferenced.
    ...
    is_yolo_model = "ultralytics" in str(type(model))
    ...
    if is_keras_model:
        for i in range((len(image_paths) + BATCH_SIZE - 1) // BATCH_SIZE):
            ...
            preds = np.where(preds > confidence, 1, 0)  # Filter out low confidence predictions
        ...
        for idx in range(0, len(image_paths), BATCH_SIZE):
            ...
            for i, r in enumerate(model.predict(batch, conf=confidence, imgsz=IMAGE_SIZE, verbose=False)):
                if hasattr(r, "masks") and r.masks is not None:
                    preds = r.masks.data.max(dim=0)[0].detach().cpu().numpy()  # Combine masks and convert to numpy
    ...
    print(f"It took {round(time.time() - start)} sec to predict with {confidence} Confidence Threshold")
    if is_keras_model and keras is not None:
